			...
			for (Map<String, String> listDetail : list.get(0)) {
				...
				if ("Y".equals(isID)) {
					insFieldOrigSql += "insert into md_field "
							+ "select SYSTEM_TYPE, TABLENM, 'ORIG_'||FIELDNM, '原始'||FIELDCNM, DATA_CAT, DATA_LENGTH, PRIMARY_KEY, "
							+ "NULL_FLAG, INIT_VALUE, ENCRYPT_FLAG, FIELD_SEQ_NO, BLURRY_FLAG, VALID_S_DATE, VALID_E_DATE, DATA_CODE, "
							+ "DATA_DESC||'原始申報資料之'||FIELDCNM, FORMULA, FIELD_LOGIC, USER_ID, TRAN_DATE "
							+ "from md_field where tablenm = '" + tableNewName + "' and fieldnm = '" + columnName + "'; \n";
					updFieldIDSql += "update md_field "
							+ "set DATA_DESC = '因應外來人口統一證號異動作業，將原始" + columnName + "與外來人口統一證號異動檔勾稽，若勾稽到則以新ID取代，否則維持原始" + columnName + "', "
							+ "FIELD_LOGIC = '與DWU_FOREIGN_ID_MAP勾稽得到，否則為原始" + columnName + "' "
